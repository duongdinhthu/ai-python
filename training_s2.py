...
train_datagen = ImageDataGenerator(
    ...
    width_shift_range=0.4,  # Increased
    height_shift_range=0.4,  # Increased
    shear_range=0.4,         # Increased
    zoom_range=0.4,          # Increased
    horizontal_flip=True,
    fill_mode='nearest',
    brightness_range=[0.5, 1.5],  # Expanded
    channel_shift_range=100.0      # Increased
)
...
# Define the model
checkpoint_path = 'best_model_s2.keras'
...
class MetricsCallback(Callback):
    def __init__(self, filename='metrics.csv', best_acc_file='best_val_accuracy_s2.txt'):
...
early_stopping = EarlyStopping(monitor='val_accuracy', patience=7, restore_best_weights=True)  # Reduced patience
...
reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=5, min_lr=1e-6)  # Reduced factor
...
model.fit(
    ...
    callbacks=[early_stopping, model_checkpoint, tensorboard_callback, reduce_lr, metrics_callback],
    initial_epoch=initial_epoch,
    class_weight=class_weights_dict  # Thêm class_weight vào đây
)

model = tf.keras.models.load_model('best_model_s2.keras')
model.save('disease_diagnosis_model_final_s2.keras')
