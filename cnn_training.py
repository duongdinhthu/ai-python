...
train_datagen = ImageDataGenerator(
    rescale=1./255,
    rotation_range=10,  # Increased
    width_shift_range=0.1,  # Increased
    height_shift_range=0.1,  # Increased
    shear_range=0.1,         # Increased
    zoom_range=0.1,          # Increased
    horizontal_flip=True,
    fill_mode='nearest',
    brightness_range=[0.9, 1.1],  # Expanded
...
class MetricsCallback(Callback):
    def __init__(self, filename='metrics.csv', best_acc_file='best_val_accurary.txt'):
...
early_stopping = EarlyStopping(monitor='val_accuracy', patience=15, restore_best_weights=True)  # Reduced patience
...
reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=15, min_lr=0.0001)  # Reduced factor
...
model.save('disease_diagnosis_model_final.keras')
